...
- [ ] 创建 `player.rs` 文件
- [ ] 定义 `Position` enum (GK, CB, LB, RB, etc.)
- [ ] 定义 `Foot` enum (Left, Right, Both)
- [ ] 定义 `PlayerStatus` enum (Healthy, Injured, etc.)
...
- [ ] 在 `player.rs` 中实现 `Player::new()`
- [ ] 实现 `age_player()` - 增加年龄，调整能力
- [ ] 实现 `recover_fatigue()` - 恢复疲劳
- [ ] 实现 `injure()` - 设置伤病
- [ ] 实现 `heal()` - 治疗伤病
- [ ] 实现 `is_gk()` - 判断是否门将
- [ ] 实现 `can_play_position()` - 判断能否踢某位置
...
- [ ] 在 `player.rs` 中实现 `calculate_overall_ability()`
- [ ] 实现 `get_position_rating()` - 计算特定位置评分
- [ ] 实现各个位置的属性权重逻辑
...
- [ ] 创建 `team.rs` 文件
- [ ] 定义 `Team` struct
- [ ] 定义 `PlayerSlot` struct
- [ ] 定义 `TeamStatistics` struct
- [ ] 添加必需字段
- [ ] 导入 `player::Position` 和 `tactics::*`
...
- [ ] 在 `team.rs` 中实现 `Team::new()`
- [ ] 实现 `add_player()` - 添加球员ID到球队
- [ ] 实现 `remove_player()` - 移除球员
- [ ] 实现 `set_starting_11()` - 设置首发阵容
- [ ] 实现 `update_statistics()` - 更新统计
...
- [ ] 在 `team.rs` 中实现 `get_team_strength()` - 计算球队整体实力
- [ ] 实现 `calculate_attack_strength()` - 进攻实力
- [ ] 实现 `calculate_defense_strength()` - 防守实力
- [ ] 需要依赖 `PlayerRepository` 来获取球员数据
- [ ] 导入 `player::Player`
...
- [ ] 创建 `league.rs` 文件
- [ ] 定义 `League` struct
- [ ] 定义 `MatchSchedule` struct
- [ ] 定义 `Round` struct
- [ ] 定义 `ScheduledMatch` struct
...
- [ ] 在 `league.rs` 中实现 `League::new()`
- [ ] 实现 `generate_schedule()` - 生成赛程（双循环）
- [ ] 实现 `get_current_round_matches()` - 获取当前轮次比赛
- [ ] 实现 `advance_round()` - 进入下一轮
...
### Task 4.1: 战术枚举定义
- [ ] 定义 `Formation` enum 及其所有变体
- [ ] 定义 `DefensiveHeight` enum
- [ ] 定义 `PassingStyle` enum
- [ ] 定义 `Tempo` enum
...
- [ ] 实现 `Formation::positions()` - 返回该阵型的位置列表
- [ ] 实现 `Formation::name()` - 返回阵型名称
- [ ] 定义每个阵型的位置布局
...
- [ ] 定义 `PlayerRole` enum（所有FM角色）
- [ ] 定义 `Duty` enum (Attack, Support, Defend, Stopper, Cover)
- [ ] 定义 `PlayerRoleAssignment` struct
...
- [ ] 实现 `get_required_attributes()` - 返回角色所需关键属性
- [ ] 实现 `get_description()` - 角色描述
- [ ] 实现 `is_suitable_for_position()` - 角色是否适合某位置
...
- [ ] 创建 `match_result.rs` 文件
- [ ] 定义 `MatchResult` struct
- [ ] 定义 `MatchMode` enum (Live, Quick)
- [ ] 定义 `MatchEvent` enum
- [ ] 添加所有必需字段
...
- [ ] 导出 `tactics` 模块
- [ ] 导出 `attributes` 模块
- [ ] 重新导出常用类型：`pub use player::{Player, Position, Foot, PlayerStatus};`
- [ ] 重新导出常用类型：`pub use team::{Team, PlayerSlot, TeamStatistics};`
- [ ] 重新导出常用类型：`pub use league::{League, MatchSchedule, Round, ScheduledMatch};`
- [ ] 重新导出常用类型：`pub use match_result::{MatchResult, MatchMode, MatchEvent};`
